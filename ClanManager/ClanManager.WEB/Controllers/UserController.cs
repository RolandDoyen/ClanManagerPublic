...
namespace ClanManager.WEB.Controllers
{
    ...
    public class UserController : BaseController<UserController>
    {
        ...
        public async Task<IActionResult> Create(UserFormViewModel model)
        {
            ...
            {
                return HandleException(ex, "User Create Error for user with Email : {Email}", model, Resources.Error_User_Create_Controller, model.Email);
            }
        }
        ...
        public async Task<IActionResult> List()
        {
            ...
            {
                return HandleException(ex, "User List Error", new UserListViewModel(), Resources.Error_User_List_Controller);
            }
        }
        ...
        public async Task<IActionResult> Detail(Guid id)
        {
            ...
            catch (NoSessionUserException ex)
            {
                ...
                _logger.LogWarning(ex, "User Detail failed for User with Id : {id}. No User in Session.", id);
            }
            catch (SessionUserNotFoundException ex)
            {
                TempData["ErrorMessage"] = ex.Message;
                _logger.LogWarning(ex, "User Detail failed for User with Id : {Id}. Session User not found.", id);
            }
            catch (WrongRoleException ex)
            {
                TempData["ErrorMessage"] = ex.Message;
                _logger.LogWarning(ex, "User Detail failed for User with Id : {Id}. Your role doesn't allow you to perform that action.", id);
            }
            catch (UserNotFoundException ex)
            {
                TempData["ErrorMessage"] = ex.Message;
                _logger.LogWarning(ex, "User Detail failed for User with Id : {id}. User not found.", id);
            }
            catch (Exception ex)
            {
                return HandleException(ex, "User Detail Error for User with Id : {Id}", new UserDetailViewModel(), Resources.Error_User_Detail_Controller, id);
            }
        ...
        public async Task<IActionResult> Login(UserFormViewModel model)
        {
            ...
            {
                return HandleException(ex, "User Login Error for User with Email : {Email}", model, Resources.Error_User_Login_Controller, model.Email);
            }
        ...
        /// <returns>Redirects to the user detail page if the role change succeeds, or redirects to the user list page if an error occurs or the session is invalid.</returns>
        [HttpPost]
        [SessionAuthorize]
        [CheckBan]
        [AuthorizeRole(Role.SuperAdmin, Role.Admin)]
        public async Task<IActionResult> ChangeRole(Guid id, Role newRole)
        {
            ...
            {
                TempData["ErrorMessage"] = ex.Message;
                _logger.LogWarning(ex, "User Change Role failed for User with Id : {Id}. No User in Session.", id);
            }
            catch (SessionUserNotFoundException ex)
            {
                TempData["ErrorMessage"] = ex.Message;
                _logger.LogWarning(ex, "User Change Role failed for User with Id : {Id}. Session User not found.", id);
            }
            catch (WrongRoleException ex)
            {
                TempData["ErrorMessage"] = ex.Message;
                _logger.LogWarning(ex, "User Change Role failed for User with Id : {Id}. Your role doesn't allow you to perform that action.", id);
            }
            catch (UserNotFoundException ex)
            {
                TempData["ErrorMessage"] = ex.Message;
                _logger.LogWarning(ex, "User Change Role failed for User with Id : {Id}. User not found.", id);
            }
            catch (RoleChangeException ex)
            {
                TempData["ErrorMessage"] = ex.Message;
                _logger.LogWarning(ex, "User Change Role failed for User with Id : {Id}.", id);
            }
            catch (Exception ex)
            {
                TempData["ErrorMessage"] = Resources.Error_User_ChangeRole_Controller;
        ...
        /// <returns>Redirects to the user detail page if the operation succeeds, or redirects to the user list page if an error occurs or the session is invalid.</returns>
        [HttpPost]
        [SessionAuthorize]
        [CheckBan]
        [AuthorizeRole(Role.SuperAdmin, Role.Admin)]
        public async Task<IActionResult> ToggleBan(Guid id)
        {
            ...
            {
                TempData["ErrorMessage"] = ex.Message;
                _logger.LogWarning(ex, "User Toggle Ban failed for User with Id : {id}. No User in Session.", id);
            }
            catch (SessionUserNotFoundException ex)
            {
                TempData["ErrorMessage"] = ex.Message;
                _logger.LogWarning(ex, "User Toggle Ban failed for User with Id : {id}. Session User not found.", id);
            ...
            {
                TempData["ErrorMessage"] = ex.Message;
                _logger.LogWarning(ex, "User Toggle Ban failed for User with Id : {id}. Your role doesn't allow you to perform that action.", id);
            }
            catch (ToggleBanException ex)
            {
                TempData["ErrorMessage"] = ex.Message;
                _logger.LogWarning(ex, "User Toggle Ban failed for User with Id : {id}. Error while toggle ban User.", id);
            }
            catch (Exception ex)
            {
                TempData["ErrorMessage"] = Resources.Error_User_ToggleBan_Controller;
                _logger.LogError(ex, ex.Message);
            }
